	...
	socialSharePrivacy = function (target, settings) {
		...
		for (i=0;i<sSP_targets.length;i++) {
			...
			if (facebook_on) {
				...
				var _fb_btn_click_ = function ( oEvent ) {
					toggleSwitch(
				...
				var _fb_dummy_btn_ = function() { 
					...
					addEvent(btn, 'click', _fb_btn_click_);
				return btn; };
			...
			if (twitter_on) {
				...
				var _twitter_btn_click_ = function ( oEvent ) { 
					toggleSwitch(
				...
				var _twitter_dummy_btn_ = function () { 
					...
					addEvent(btn, 'click', _twitter_btn_click_); 
				return btn;};
				var sSP_LI = createElement('li', { 
			...
			if (gplus_on) {
				...
				var _gplus_btn_click_ = function ( oEvent ) { 
					toggleSwitch(
				...
				var _gplus_dummy_btn_ = function () { 
					...
					addEvent(btn, 'click', _gplus_btn_click_);
				return btn; };
				var sSP_LI = createElement('li', { 
			...
			if ( ( (facebook_on && facebook_perma)
				...
				for (var m=0;m<inputs.length;m++) {
					addEvent(inputs[m], 'change', function (event) {
						var click = this.id;
						console.log(click);
						var service = ( (click.indexOf('facebook') > 0 ) ? 'facebook' : 
							( (click.indexOf('twitter') > 0 ) ? 'twitter' : 
								( (click.indexOf('gplus') > 0 ) ? 'gplus' : false 
						)));
						if (!service) return;
				...
				*/
				...
				if (facebook_on && facebook_perma && cookies.socialSharePrivacy_facebook === 'perma_on') {
					_gplus_btn_click_(); // $('li.facebook span.switch', context).click();
				}
				if (twitter_on && twitter_perma && cookies.socialSharePrivacy_twitter === 'perma_on') {
					_gplus_btn_click_(); // $('li.twitter span.switch', context).click();
				}
				if (gplus_on && gplus_perma && cookies.socialSharePrivacy_gplus === 'perma_on') {
					_gplus_btn_click_(); // $('li.gplus span.switch', context).click();
				}
			}
